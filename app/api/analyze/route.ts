...
export async function POST(request: Request) {
  try {
    ...
    // Read the secret when the request is handled. This route intentionally uses
    // Next.js' default Node.js runtime so Vercel supplies runtime environment
    // variables directly through process.env rather than an Edge bundle.
    const apiKey = process.env.OPENAI_API_KEY?.trim();
    if (!apiKey) {
      console.error("Analyze route configuration error: OPENAI_API_KEY is missing or empty", {
        vercelEnvironment: process.env.VERCEL_ENV ?? "local",
        vercelRegion: process.env.VERCEL_REGION ?? "unknown"
      });
      return NextResponse.json({ error: "The analysis service is not configured yet." }, { status: 503 });
    }
